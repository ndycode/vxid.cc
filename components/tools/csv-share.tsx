...
import { Copy, Check, Table, CaretDown, Lock, Eye, EyeSlash } from "@phosphor-icons/react";
import { Input } from "@/components/ui/input";
import { EXPIRY_OPTIONS } from "@/lib/share-types";

export function CsvShare() {
            ...
            {result ? (
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center">
                            <Table weight="duotone" className="w-5 h-5 text-primary" />
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="font-semibold">CSV shared!</p>
                ...
                <div className="space-y-4">
                    <textarea
                        ...
                        className="w-full h-28 px-3 py-2 text-sm font-mono bg-muted/50 border rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-primary/50"
                    />
                    ...
                    {hasData && (
                        <div className="space-y-1">
                                        ...
                                        {rows.map((row, i) => (
                                            <tr key={i} className={i === 0 ? "bg-muted/50 font-semibold" : ""}>
                                                {row.split(',').map((cell, j) => (
                                                    <td key={j} className="border px-2 py-1 truncate max-w-[100px]">
                                                        {cell.trim()}
                                                    </td>
                                                ))}
                                            </tr>
                            ...
                            <p className="text-xs text-muted-foreground">
                                {content.split('\n').filter(Boolean).length} rows · {rows[0]?.split(',').length || 0} columns
                            </p>
                        </div>
                    )}
